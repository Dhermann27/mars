...
class Controller extends BaseController
{
    ...
    private $steps = array('campersSelected' => 0, 'isAddressCurrent' => null, 'isCamperDetail' => null,
        ...
        'nametagsCustomized' => null, 'medicalResponsesNeeded' => null, 'username' => null);
    ...
    protected function getStepData($id = null)
    {
        if (!isset(Auth::user()->camper)) return $this->steps;
        if (!$id) $id = Auth::user()->camper->id;
        $camper = Camper::find($id);
        $this->steps["username"] = $camper->firstname . ' ' . $camper->lastname;
        if ($camper->email) $this->steps["username"] .= ' &lt;' . $camper->email . '&gt;';
    ...
    private function runPostPaymentChecks($yearsattending, $campers): void
    {
        ...
        if ($this->steps["isRoomsSelected"]) {
            ...
            $children = $campers->filter(function ($camper) use ($yearsattending) {
                return Carbon::parse($camper->birthdate)->diffInYears($this->year->checkin) < 18 &&
                    $yearsattending->contains('camper_id', $camper->id);
            });
            if ($children->count() > 0) {
